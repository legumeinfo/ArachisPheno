...
def detail_accession(request, pk=None):
    ...
    phenotype_table = AccessionPhenotypeTable(pk,Phenotype.objects.annotate(num_values=Count('phenotypevalue')).published().filter(phenotypevalue__obs_unit__accession_id=pk), order_by="name")
    RequestConfig(request, paginate={"per_page":20}).configure(phenotype_table)
    variable_dict = {}
    variable_dict["phenotype_table"] = phenotype_table
